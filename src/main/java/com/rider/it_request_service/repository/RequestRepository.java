...
public interface RequestRepository extends JpaRepository<Request, Integer> {
    ...
    @Query(
            "SELECT r.requestNumber FROM Request r WHERE r.requestNumber LIKE :pattern ORDER BY r.requestNumber DESC LIMIT 1")
    ...
    List<Request> findRequestsByDateRange(
    ...
    @Query(
            "SELECT COUNT(r) FROM Request r WHERE WEEK(r.createdAt) = WEEK(CURRENT_DATE) AND YEAR(r.createdAt) = YEAR(CURRENT_DATE)")
    long countRequestsThisWeek();

    // คำขอที่สร้างในเดือนนี้
    @Query(
            "SELECT COUNT(r) FROM Request r WHERE MONTH(r.createdAt) = MONTH(CURRENT_DATE) AND YEAR(r.createdAt) = YEAR(CURRENT_DATE)")
    ...
    @Query(
            "SELECT AVG(requestCount) FROM (SELECT COUNT(r) AS requestCount FROM Request r GROUP BY r.userId) AS requestCounts")
