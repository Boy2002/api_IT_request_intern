...
public class RequestService {
    ...
    public String generateRequestNumber() {
        ...
        Optional<String> lastRequestOpt =
                requestRepository.findLastRequestNumber("REQ-" + datePart + "-%");
    ...
    public List<RequestAdminBoardDTO> getRequestByDate(
    ...
    public RequestHistoryAdminBoardDTO updateStatus(
            CustomUserDetails user, RequestStatusHistory requestStatusHistory) {
        ...
        Request request =
                requestRepository
                        .findById(requestStatusHistory.getRequestId())
                        .orElseThrow(
                                () ->
                                        new GlobalExceptionHandler.ResourceNotFoundException(
                                                "Request not found with ID: "
                                                        + requestStatusHistory.getRequestId()));
        ...
        List<RequestStatusHistory> refStatusHistory =
                requestStatusHistoryRepository.findByrequestId(requestStatusHistory.getRequestId());
    ...
    public List<RequestDTO> getRequestToWeek() {
        ...
        List<Request> listRequest =
                requestRepository.findRequestsByDateRange(startOfWeek, endOfWeek);
        return listRequest.stream().map(RequestMapper.INSTANCE::toDTO).toList();
    }
    ...
    public List<RequestDTO> getRequestToMonth() {
        ...
        List<Request> listRequest =
                requestRepository.findRequestsByDateRange(startOfMonth, endOfMonth);
        return listRequest.stream().map(RequestMapper.INSTANCE::toDTO).toList();
    }
    ...
    public Map<String, Object> getRequestStatistics() {
        ...
        statistics.put(
                "pendingRequests",
                requestRepository.countByStatus(Request.Status.PENDING)); // คำร้องใหม่
        statistics.put(
                "inProgressRequests",
                requestRepository.countByStatus(Request.Status.IN_PROGRESS)); // ระหว่างดำเนินการ
        statistics.put(
                "resolvedRequests",
                requestRepository.countByStatus(Request.Status.RESOLVED)); // ดำเนินการเรียบร้อย
        statistics.put(
                "rejectedRequests",
                requestRepository.countByStatus(Request.Status.REJECTED)); // ปฏิเสธคำร้อง
    ...
    //        subquery.where(cb.like(cb.lower(userRoot.get("name")), "%" +
    // searchDto.name().toLowerCase() + "%"));
    //
    ...
    ////        predicates.add(cb.like(cb.lower(categoryJoin.get("categoryId")), "%" +
    // searchDto.categoryId().toLowerCase() + "%"));
    ...
    public Page<RequestAdminBoardDTO> searchRequests(SearchRequestDTO searchDto) {
        ...
        if (searchDto.requestNumber() != null) {
            predicates.add(
                    cb.like(root.get("requestNumber"), "%" + searchDto.requestNumber() + "%"));
        }
        ...
        if (searchDto.name() != null) {
            baseQuery +=
                    " AND r.userId IN (SELECT u.userId FROM User u WHERE LOWER(u.name) LIKE :name)";
        }
        if (searchDto.requestNumber() != null) {
    ...
    public List<RequestDTO> getRequestsByStatus(Request.Status status) {
        ...
        return requests.stream().map(RequestMapper.INSTANCE::toDTO).collect(Collectors.toList());
    }
